...
print('ROOT_METHODS', [m for m in dir(r) if any(k in m.lower() for k in ['overlap','contain','radius','rad','add_error','absolute','abs','center'])])
print('REAL_METHODS', [m for m in dir(r.real()) if any(k in m.lower() for k in ['lower','upper','end','radius','rad','add_error','contain','center'])])
try:
...
try:
    z1 = CBF(1); z1.add_error(RBF(1)/4)
    z2 = CBF(1); z2.add_error(RBF(1)/8)
    print('OVERLAPS', z1.overlaps(z2))
    print('CONTAINS', z1.contains(z2), z2.contains(z1))
except Exception as exc:
    print('BALL_RELATION_ERROR', repr(exc))
